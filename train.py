...
class user:
	def _init_(self, uid, uname, age, gender, email, phone):
		self.uid = uid
		self.uname = uname
		self.age = age
		self.gender = gender
...
class train:
	def _init_(self, name, number, source, dest, dep, arr):
		self.name = name
		self.number = number
		self.source = source
		self.dep = dep
	...
	def seatBook(self, date, travelClass, seatNum, conn, tempuser):
		...
		if classList is not None:
			if len(classList) > 0:
				...
				conn.sendall("No seats available for the selected class and date.\n")